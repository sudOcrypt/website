...
import { Loader2, Ban, UserX, Shield, Search, X, Plus, Trash2, Globe, User as UserIcon, RefreshCw } from 'lucide-react';
...
export function AdminUsers() {
  ...
  useEffect(() => {
    ...
    // Refresh when page becomes visible (tab switch, window focus)
    ...
    <div>
      <div className="flex flex-wrap gap-2 mb-6">
        ...
        <button
          onClick={() => loadData()}
          disabled={isLoading}
          className="ml-auto flex items-center gap-2 px-4 py-2 bg-gray-800/50 text-gray-400 border border-white/10 rounded-lg font-medium hover:text-white hover:border-cyan-500/50 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-red-500/20 text-red-400 border border-red-500/50 rounded-lg font-medium hover:bg-red-500/30 transition-all"
        >
                      ...
                      <td className="py-4 px-4">
                        {user.last_ip_address ? (
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-gray-400 font-mono text-sm">{user.last_ip_address}</span>
                            {user.is_vpn_user && (
                              <span 
                                ...
                                title={user.isp_organization ? `VPN/Proxy: ${user.isp_organization}` : 'VPN/Proxy detected'}
                              >
                          ...
                          <span className="text-gray-600 text-sm italic">Not logged in yet</span>
                        )}
                      </td>
                      <td className="py-4 px-4">
                        {user.is_admin ? (
